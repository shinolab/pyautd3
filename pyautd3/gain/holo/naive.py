...
class Naive(Holo["Naive"]):
    option: NaiveOption

    def __init__(self: Self, foci: Iterable[tuple[np.ndarray, Amplitude]], option: NaiveOption) -> None:
        super().__init__(foci)
        self.option = option
    ...
    def _gain_ptr(self: Self, _: Geometry) -> GainPtr:
        ...
        amps = np.fromiter((d.pascal() for _, d in self.foci), dtype=ctypes.c_float)
        return GainHolo().gain_holo_naive_sphere(
