...
def test_transducer_table():
    with create_emulator() as emulator:
        ...
        assert np.array_equal(table["x[mm]"].to_numpy(), np.ravel([[tr.position[0] for tr in dev] for dev in emulator]))
        assert np.array_equal(table["y[mm]"].to_numpy(), np.ravel([[tr.position[1] for tr in dev] for dev in emulator]))
        assert np.array_equal(table["z[mm]"].to_numpy(), np.ravel([[tr.position[2] for tr in dev] for dev in emulator]))
